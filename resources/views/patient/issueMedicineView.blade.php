            ...
            <div class="box-body">
                ...
                <div class="form-group">
                    ...
                    <input type="number" required class="form-control mt-2" name="patientid"
                        ...
                        placeholder="{{ __('Enter Registration or Appointment Number') }}" />
                </div>
                <div class="form-group">
                    <button type="submit" class="btn btn-info" onclick="issuemedicinefunction1()">{{__('Check')}}</button>
                </div>
...
<div class="box box-info" id="issuemedicine2" style="display:none">
            <div class="box-header with-border">
                <h3 class="box-title">{{__('Approved to Issue Medicine')}}</h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body mt-0">
                <h4>{{__('Registration No')}} : <span id="patient_id"></span></h4>
                <h4>{{__('Patient Name')}} : <span id="p_name"></span></h4>
                <h4>{{__('Appointment No')}} &nbsp;: <span id="p_appnum"></span></h4>
                <button id="btn-issue" type="button" class="btn btn-primary btn-lg mt-3 text-center"
                    value="{{ __('Issue Medicine Now') }}" onclick="go()" id="btn">{{__('Issue Medicine Now')}}</button>
                <button onclick="cancel()" class="btn btn-warning btn-lg mt-3 text-center">{{__('Cancel')}}</button>
            </div>
            <!-- /.box-body -->
            <div class="box-footer">
            </div>
            ...
            $.ajax({
                ...
                success: function (im) {
                    if(im.exist){
                        ...
                        alert("{{ __('Please Enter Today Prescriptions\' Details only!') }}");
                    }
                }
            });
            }else{
